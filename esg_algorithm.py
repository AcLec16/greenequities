...
def calculate_esg_score(company_data, employee_data):
    report = {}
        ...
        reports["selected_locations"] = company_data["selected_locations"]
        ...
        report["value_based_resource_efficiency_s"] = (f"Value-Based Resource Efficiency (value_based_resource_efficiency) for every ₹1 of stressed resources: {value_based_resource_efficiency:.2f}")
            ...
            return 2 if vbre <= 0.75 else 3
        elif 1.01 <= vbre <= 1.5:
            return 4
        elif 1.51 <= vbre <= 2.0:
            return 5 if vbre <= 1.75 else 6
        elif 2.01 <= vbre <= 3.0:
            return 7
        elif 3.01 <= vbre <= 5.0:
            return 8
        ...
        enviro_comp = (selected_enviro / 10) * 10
        ...
        report["bath_tubs_full"] = (water_cost / 0.05) / 302
    ...
    turnover_rating = calculate_turnover_rating(turnover_rate)
        ...
        selected_data_law_rating = (selected_data_laws / 10) * 10
        ...
        company_data["social_impact_partnerships"] = company_data["social_impact_partnerships"]
        
         # 2. Supply chain ESG
        supplier_retention_score = (company_data["remaining_suppliers"] / company_date["all_time_suppliers"]) * 10
